...
// Eksekusi update
$query = mysqli_query($konek, "UPDATE disposisi SET status='$status' WHERE id='$id'");

if ($query && mysqli_affected_rows($konek) > 0) {
    echo "<script>alert('Status berhasil diubah!'); window.location.href='?page=disposisi_read';</script>";
} else {
