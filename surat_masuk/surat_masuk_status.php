...
// Eksekusi update
$query = mysqli_query($konek, "UPDATE surat_masuk SET status='$status' WHERE id='$id'");

if ($query && mysqli_affected_rows($konek) > 0) {
    echo "<script>alert('Status berhasil diubah!'); window.location.href='?page=surat_masuk_read';</script>";
} else {
